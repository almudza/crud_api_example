<template>
  <div>
      ...
      <form @submit.prevent="addArticle" class="mb-3">
        ...
        <div class="form-group">
          <textarea class="form-control" placeholder="Body" v-model="article.body" cols="30" rows="10"></textarea>
        </div>
      ...
      <div class="card  card-body mb-3" v-for="article in articles" v-bind:key="article.id" >
        ...
        <p> {{ article.body }} </p>
        <hr>
        <button @click="editArticle(article)" class="btn btn-outline-warning btn-block">Edit</button>
        <button @click="deleteArticle(article.id)" class="btn btn-outline-danger btn-block">Delete</button>
      </div>
  </div>
</template>
